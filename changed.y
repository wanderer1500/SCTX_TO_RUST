...
int     var_count = 0;
 ...
 void add_var(const char *name, const char *type, int is_output) {
 ...
 int is_input_var(const char *name) {
...
char chart_name[256] = "";
 ...
 StateInfo *find_state(const char *name, const char *parent) {
 ...
 StateInfo *get_current_state(void) {
...
#define SET_TRANS_SRC(t) \
 ...
 void parse_transition_cond(TransInfo *t, char *cond) {
 ...
 void add_transition(int has_if, char *cond, int is_immediate, int is_delayed, int is_auto, int is_default, int is_join, char *do_stmts, char *target_state, int mod) {
 ...
 void add_state_entry(const char *name, const char *parent, int is_init, int is_final) {
 ...
 void enum_name_for(const char *scope, char *out) {
    if (strcmp(scope, chart_name) == 0) strcpy(out, "TopState");
    else sprintf(out, "%sState", cap_str(scope));
}

 void field_name_for(const char *scope, char *out) {
    char *l = lcase(scope);
    sprintf(out, "%s_state", l);
    free(l);
}

 const char *initial_child(const char *scope) {
    for (int i = 0; i < state_count; i++)
 ...
 const char *final_child(const char *scope) {
    for (int i = 0; i < state_count; i++)
 ...
 int child_count(const char *scope) {
    int n = 0;
    for (int i = 0; i < state_count; i++)
 ...
 int feat_has_shallow  = 0;
 int feat_has_deep     = 0;
 int feat_has_deferred = 0;
 int feat_has_counter  = 0;
 int feat_has_delayed  = 0;
 int feat_has_regions  = 0;
 ...
 int count_cond_vars(char seen[][256]) {
    int nseen = 0;
    for (int i = 0; i < trans_count; i++) {
